...
function proxy(context, obj, callback) {
  ...
  function deepProxy(obj) {
    ...
    for (let key in obj) {
      ...
      if (type(value) == 'object') {
        defineAttr(r, key, deepProxy(value))
      } else if (type(value) == 'array') {
        defineAttr(r, key, proxyArray(value))
      } else {
        defineAttr(r, key, value)
      }
    }
    return r
  }
  ...
  function proxyArray(array) {
    ...
    array.forEach((item, index) => {
      if (type(item) == 'object') {
        defineAttr(r, index, deepProxy(item))
      } else if (type(item) == 'array') {
        defineAttr(r, index,  proxyArray(item))
      } else {
        defineAttr(r, index, item)
      }
    })
    return r
  }
